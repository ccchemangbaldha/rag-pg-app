...
import { MessageSquare, Bot, User as UserIcon, Calendar, Database } from "lucide-react";
...
export const HistoryViewer = ({ logs }: { logs: HistoryLog[] }) => {
	...
	return (
		<div className="space-y-6">
			{logs.map((log) => {
				// Check if we have the new metadata structure with sections
				// Type casting or safe check for metadata
				const metadata: any = log.metadata || {};
				const hasSections = metadata.sections && Array.isArray(metadata.sections);
					...
					<div key={log.historyId} className="relative pl-6 border-l-2 border-gray-200 dark:border-gray-700 pb-6 last:pb-0">
						...
						<div className="space-y-3">
							...
							<div className="flex gap-3 flex-row-reverse">
								...
								<div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/30 p-3 rounded-2xl rounded-tr-none text-sm text-gray-800 dark:text-gray-200 w-full">
									{hasSections ? (
										<div className="space-y-3">
											{metadata.sections.map((section: any, idx: number) => (
												<div key={idx} className={idx > 0 ? "pt-2 border-t border-blue-200 dark:border-blue-800" : ""}>
													<div className="mb-1">{section.text || section.botOutput}</div>
													{section.sql && (
